...
public class PreferenceHelper {
    ...
    public void SetPrefs(SharedPreferences.Editor editor, EditText name, EditText phone, EditText email, EditText password, String gender, String notification, String time) {
        if (name.getText().length() > 0) editor.putString("name", name.getText().toString());
        if (email.getText().length() > 0) editor.putString("email", email.getText().toString());
        if (phone.getText().length() > 0) editor.putString("phone", phone.getText().toString());
        if (gender.length() > 0) editor.putString("gender", gender);
        if (notification.length() > 0) editor.putString("notification", notification);
        if (password.getText().length() > 0)
            editor.putString("password", password.getText().toString());
        if (time.length() > 0) editor.putString("time", time);
        editor.apply();
    }
    ...
    public void DisplayEdit(SharedPreferences prefs, EditText name, TextView phone, EditText email, EditText password, RadioButton gender, RadioButton gender2, RadioButton notification, RadioButton notification2, TextView txt) {
        ...
        email.setHint(" " + Email);
        email.setText("");
        if (Gender.equals("Male")) gender.setChecked(true);
        else if (Gender.equals("Female")) gender2.setChecked(true);
        if (Notification.equals("Enabled")) notification.setChecked(true);
        else notification2.setChecked(true);

        String temp = "•".repeat(Password.length());
        password.setHint(" " + temp);
        if (Password.equals("Not Set")) password.setHint(" " + Password);
        password.setText("");
        txt.setText(Time);
    }
    ...
    public String validate(EditText name, EditText phone, EditText email, EditText password) {
        if (name.length() > 0)
            if (!name.getText().toString().matches("[a-zA-z ]+")) return "Invalid Name";
        if (email.length() > 0) if (!Patterns.EMAIL_ADDRESS.matcher(email.getText()).matches())
            return "Invalid Email Address";
        if (phone.length() > 0) if (!phone.getText().toString().matches("\\+?\\d{10,12}"))
            return "Invalid Phone Number";
        if (password.length() > 0) if (password.length() < 8) return "Min Password Length = 8";
        return "";
    }
